 ...
 * @author M.B Stocks
 */
public class TemporarySortedFastaDecoder implements Decoder<byte[]> {
  public static final byte LF = 10;

  private final byte delimiter;
  
  public TemporarySortedFastaDecoder() {
   this.delimiter = '>';
  }
  /**
   * 
  ...
  public byte[] decode( ByteBuffer buffer )
  {
    ...
    while ( buffer.hasRemaining() )
    {
      ...
      if(b == delimiter)
      {
        //fast forward the end of the sorted file because a > was encountered 
        while ( buffer.hasRemaining() )
          b = buffer.get();
      }
      if ( b == LF )
  ...
  public byte[] parseRow(ByteBuffer buffer) {
    ...
    while (buffer.hasRemaining()) {
      ...
      if ( b == LF) {
        ...
        fields = parseField(buffer.duplicate(), fieldNumber, fieldEndPos - fieldStartPos - 1);
        fieldNumber++;
        // reset main buffer
