...
public final class NavigableImagePanel extends JPanel
{
  ...
  private static final double NAV_IMAGE_FACTOR = 0.4; // 30% of panel's width
  ...
  private String myHairpinSequence;
  private boolean onlyNavigationImage = false;
  ...
  private class WheelZoomDevice implements MouseWheelListener
  {
    ...
    public void mouseWheelMoved( MouseWheelEvent e )
    {
      ...
      if ( isInNavigationImage( p ) )
      {
        ...
        //System.out.println("navzoomfactor " + navZoomFactor);
        zoomNavigationImage();
      }
      else
      {
        if ( isInImage( p ) )
        {
          ...
          //System.out.println("navzoomfactor " + zoomFactor);
          zoomImage();
        }
      }
    }
  ...
  public void setHairpinSequence( String newSequence )
  {
    myHairpinSequence = newSequence;
  }
  ...
  public void setZoomDevice( ZoomDevice newZoomDevice )
  {
    ...
    {
      if ( newZoomDevice == ZoomDevice.MOUSE_BUTTON )
      {
        removeWheelZoomDevice();
        addButtonZoomDevice();
      }
      ...
      {
        if ( newZoomDevice == ZoomDevice.MOUSE_WHEEL )
        {
          removeButtonZoomDevice();
          addWheelZoomDevice();
        }
...
//                            RenderingHints.VALUE_INTERPOLATION_BICUBIC);  
  ...
  public BufferedImage getNavigationImage()
  {
  ...
  public void setOnlyNavigationImage( boolean enabled )
  {
    this.onlyNavigationImage = enabled;
    repaint();
  }
  ...
  {
    double temp = scale / initialScale;
  ...
  private void zoomNavigationImage()
  {
    ...
    //System.out.println("navscale: " + navScale);
    repaint();
  }
  ...
  protected void paintComponent( Graphics g )
  {
    ...
    if ( isNavigationImageEnabled() )
    {
      ...
      g2.setRenderingHint( RenderingHints.KEY_ANTIALIASING,
        RenderingHints.VALUE_ANTIALIAS_ON );
      g2.drawImage( navigationImage, 0, 0, getScreenNavImageWidth(),
        getScreenNavImageHeight(), null );
      drawZoomAreaOutline( g2 );
      drawNavImageBorder( g2 );
    }
  }
  ...
  private void drawZoomAreaOutline( Graphics g )
  {
    ...
    //Rectangle2D newRect = new Rectangle2D.Double(0.0, 0.0, width, height);
    g.setColor( Color.RED );
    g.drawRect( x, y, width, height );
  }

  private void drawNavImageBorder( Graphics2D g )
  {


    int x = -originX * getScreenNavImageWidth() / getScreenImageWidth();
    int y = -originY * getScreenNavImageHeight() / getScreenImageHeight();
    int width = getScreenNavImageWidth();
    int height = getScreenNavImageHeight();

    RoundRectangle2D newRect = new RoundRectangle2D.Double( 1.0, 1.0, width, height, 1.0, 1.0 );
